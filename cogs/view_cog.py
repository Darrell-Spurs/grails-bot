...
from utils.collection_ui import CardView, render_card
...
async def _card_autocomplete(interaction: discord.Interaction, current: str):
    ...
    try:
        cards = await asyncio.to_thread(db.search_user_cards, owner_id, query=query,
                                        limit=25, variant=variant, artist=artist)
    except Exception:
...
class ViewCog(commands.Cog):
    ...
    @app_commands.autocomplete(item=_card_autocomplete, variant=_variant_autocomplete,
                               artist=owned_artist_autocomplete)
    async def view(self, ctx, user: Optional[discord.User] = None,
        ...
        card, note = await asyncio.to_thread(self._resolve, owner.id, item,
                                             variant=variant, artist=artist)
        if note:
            await ctx.send(note)
            return

        embed, art = await render_card(card, owner_name, owner_icon=owner.display_avatar.url,
                                       owner_id=owner.id)
